...
@push('title') Logger @endpush
@push('data-master') open active @endpush 
@push('selected_dm') <span class="selected"></span>@endpush 
@push('open_dm') open @endpush

@push('active_logger') open active @endpush 
@push('selected_log')<span class="selected"></span>@endpush
@section('content')
<!-- BEGIN PAGE HEAD-->
<div class="page-head">
    <!-- BEGIN PAGE TITLE -->
            ...
            <div class="portlet-body">
                <div class="table-toolbar">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="btn-group">
                                <button class="btn sbold green modal-logger" data-name="modal-logger"> Add New
                ...
                <table class="table table-striped table-bordered table-hover table-checkable order-column dataTable no-footer" id="table-logger">
                    <thead>
                        <tr>
                            <th width="5%"> No </th>
...
<script>
    ...
    //modal
    $(document).on('click', '.modal-logger', function() {
        showLoading()
		$.ajax({
			url: '/modal-dm',
            type: 'GET',
            ...
            case'save-logger':
                $.ajax({
                    ...
                    success: function (data) {
                        $('#myModal').modal('hide');
                        $('#table-logger').DataTable().ajax.reload();
                        swal({
                            title :'Sukses',
	            ...
	            confirm: {
	                ...
	                callback: function(){
	                    ...
	                    $.ajax({
	                        ...
	                        data: $("#form").serialize(),
	                        success: function(data) {
	                            var tabel = $('#table-logger').DataTable();
	                            tabel.ajax.reload();
                                if (data.errors) {
                                    swal({
                                        title :'Gagal',
